...
const processBounce = async (uuid: string, bounceData: BounceData): Promise<APIGatewayProxyResult> => {
  try {
...
export const postBounceHandler = async (event: APIGatewayEvent): Promise<APIGatewayProxyResult> => {
  ...
  try {
    const messageId = event.pathParameters?.messageId as string
    const bounceData = extractBounceDataFromEvent(event)
    return await processBounce(messageId, bounceData)
  } catch (error: unknown) {
